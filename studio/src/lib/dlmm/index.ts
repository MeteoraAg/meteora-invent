...
/**
 * Claim all swap fees + LM rewards for the wallet's positions on a DLMM pool.
 */
export async function claimAllFees(
  ...
  for (const position of userPositions) {
    console.log(
      `- Position ${position.publicKey.toString()}: feeX=${position.positionData.feeX.toString()} feeY=${position.positionData.feeY.toString()}`
    );
  }

  let claimTxs;
  try {
    claimTxs = await dlmmPool.claimAllRewards({
      owner: wallet.publicKey,
      positions: userPositions,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.log(`> Nothing to claim: ${message}`);
    return;
  }
